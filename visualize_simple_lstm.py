...
def load_simple_model(model_path, input_size, device):
    """Load trained SimpleLSTM model"""
    try:
        model = SimpleLSTMModel(input_size, hidden_size=128).to(device)
...
def visualize_simple_lstm(symbol='SOL', lookback=60):
    ...
    try:
        ...
        # Load model
        model_path = f'models/saved_models/{symbol}_simple_model.pth'
        
        if not os.path.exists(model_path):
            logger.error(f"Model file not found: {model_path}")
            logger.error(f"Please train first: python train_simple_lstm.py --symbol {symbol}")
            return
        ...
        save_path = os.path.join(output_dir, f'{symbol}_simple_lstm_analysis.png')
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Visualize Simple LSTM Model Predictions')
    ...
    visualize_simple_lstm(args.symbol)
