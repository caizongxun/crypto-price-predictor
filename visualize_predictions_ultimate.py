#!/usr/bin/env python3
"""
Visualize Ultimate Ensemble Model Predictions
...
def load_ultimate_model(model_path, input_size, device):
    """Load trained Ultimate Ensemble model"""
    try:
        model = UltimateEnsembleModel(input_size=input_size).to(device)
...
def visualize_ultimate(symbol='SOL', lookback=60):
    """Visualize Ultimate model predictions with correct scaling"""
    ...
    try:
        ...
        if not os.path.exists(model_path):
            ...
            logger.error(f"Please train first: python train_model_ultimate.py --symbol {symbol}")
            return
        ...
        ax1.plot(range(last_n), predictions[-last_n:], label='Predicted Price (Ultimate, USD)', color='orange', alpha=0.7, linestyle='--', linewidth=2)
        ...
        ax2.set_title('Prediction Error Distribution (Ultimate)', fontsize=12, fontweight='bold')
        ...
        ax3.set_title('Actual vs Predicted Values (Ultimate, Original Scale)', fontsize=12, fontweight='bold')
        ...
        ax4.set_title('Prediction Error Over Time (Ultimate)', fontsize=12, fontweight='bold')
